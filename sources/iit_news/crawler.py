#!/usr/bin/env python3
"""
Illinois Institute of Technology News 爬虫
从 https://www.iit.edu/news 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
IIT_URL = "https://www.iit.edu/news"
RAW_DIR = "data/raw/iit_news"
...
def categorize(title):
    ...
    categories = {
        'law': ['copyright', 'law', 'legal', 'policy'],
...
def crawl_iit():
    """从 IIT News 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Illinois Institute of Technology News...")
    print(f"📡 网址: {IIT_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(IIT_URL, headers=headers, timeout=20)
        ...
        # 查找所有文章 - article.listing-item
        article_items = soup.find_all('article', class_='listing-item')
        ...
        for item in article_items:
            try:
                # 标题和链接
                title_elem = item.find('h2', class_='listing-item__title')
                if not title_elem:
                    continue
                ...
                # 摘要
                summary_elem = item.find('div', class_='listing-item__content')
                summary_en = ""
                if summary_elem:
        ...
        output = {
            'source': 'Illinois Institute of Technology News',
            'source_url': IIT_URL,
