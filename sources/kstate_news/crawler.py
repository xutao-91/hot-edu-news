#!/usr/bin/env python3
"""
Kansas State University News 爬虫
从 https://www.k-state.edu/news/articles/ 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
KSTATE_URL = "https://www.k-state.edu/news/articles/"
RAW_DIR = "data/raw/kstate_news"
...
def categorize(title):
    ...
    categories = {
        'documentary': ['documentary', 'film', 'screening'],
        'community': ['community', 'health', 'well-being', 'food'],
        'research': ['research', 'study', 'discovery'],
        'students': ['student', 'experience', 'education']
    }
...
def crawl_kstate():
    """从 K-State News 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Kansas State University News...")
    print(f"📡 网址: {KSTATE_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(KSTATE_URL, headers=headers, timeout=20)
        ...
        for item in article_items:
            try:
                ...
                if href.startswith('http'):
                    ...
                    url = f"https://www.k-state.edu{href}"
