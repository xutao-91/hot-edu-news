#!/usr/bin/env python3
"""
Indiana University News 爬虫
从 https://news.iu.edu/news/ 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
IU_NEWS_URL = "https://news.iu.edu/news/"
RAW_DIR = "data/raw/iu_news"
...
def categorize(title):
    ...
    categories = {
        ...
        'faculty': ['faculty', 'fellow', 'professor'],
        'education': ['education', 'student', 'program'],
        'award': ['award', 'fellow', 'recognized']
    }
...
def crawl_iu_news():
    """从 Indiana University News 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Indiana University News...")
    print(f"📡 网址: {IU_NEWS_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(IU_NEWS_URL, headers=headers, timeout=20)
        ...
        # 查找所有文章卡片
        article_items = soup.find_all('div', class_='rvt-card')
        ...
        for item in article_items:
            try:
                # 标题和链接
                title_elem = item.find('h3', class_='rvt-card__title')
                if not title_elem:
                    continue
                ...
                # 分类标签
                category_elem = item.find('span', class_='lw_item_category')
                content_type = category_elem.get_text(strip=True) if category_elem else 'News'
                
                # 日期
                date_str = ""
                meta_div = item.find('div', class_='rvt-c-card__meta')
                if meta_div:
