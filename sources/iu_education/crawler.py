#!/usr/bin/env python3
"""
Indiana University School of Education 爬虫
从 https://education.indiana.edu/news/index.html 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
IU_EDU_URL = "https://education.indiana.edu/news/index.html"
RAW_DIR = "data/raw/iu_education"
...
def categorize(title):
    ...
    categories = {
        'faculty': ['faculty', 'professor', 'fellow'],
        'psychology': ['psychology', 'counseling'],
        'education': ['education', 'student', 'program'],
        'award': ['elected', 'fellow', 'recognized']
    }
...
def crawl_iu_education():
    """从 IU School of Education 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Indiana University School of Education...")
    print(f"📡 网址: {IU_EDU_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(IU_EDU_URL, headers=headers, timeout=20)
        ...
        for item in article_items:
            try:
                # 标题和链接
                title_elem = item.find('h1', class_='title')
                if not title_elem:
                    continue
                ...
                if href.startswith('http'):
                    ...
                    # 相对路径转换为绝对路径
                    url = f"https://education.indiana.edu/news/{href}"
