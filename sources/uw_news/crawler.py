...
"""
UW-Madison News 爬虫
从 https://news.wisc.edu/latest-news/ 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
UW_NEWS_URL = "https://news.wisc.edu/latest-news/"
RAW_DIR = "data/raw/uw_news"
...
def crawl_uw_news():
    """从 UW-Madison News 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 UW-Madison News...")
    print(f"📡 网址: {UW_NEWS_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(UW_NEWS_URL, headers=headers, timeout=20)
        ...
        # 查找所有文章行
        article_items = soup.find_all('div', class_='story-row')
        ...
        for item in article_items:
            try:
                # 标题和链接
                title_elem = item.find('h2', class_='headline')
                if not title_elem:
                    continue
                ...
                # 摘要
                summary_elem = item.find('p', class_='lead')
