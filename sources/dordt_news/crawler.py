#!/usr/bin/env python3
"""
Dordt University News 爬虫
从 https://www.dordt.edu/news 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
DORDT_URL = "https://www.dordt.edu/news"
RAW_DIR = "data/raw/dordt_news"
...
def categorize(title):
    ...
    categories = {
        'career': ['career', 'graduates', 'outcomes', 'graduation'],
        'education': ['education', 'students', 'learning'],
        'community': ['community', 'leaders', 'relationships'],
...
def crawl_dordt():
    """从 Dordt News 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Dordt University News...")
    print(f"📡 网址: {DORDT_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(DORDT_URL, headers=headers, timeout=20)
        ...
        # 查找所有文章 - article.news-item
        article_items = soup.find_all('article', class_='news-item')
        
        print(f"✅ 找到 {len(article_items)} 篇文章，开始过滤...")
        
        for item in article_items:
            try:
                # 标题 - from aria-label or h2
        ...
        output = {
            'source': 'Dordt University News',
            'source_url': DORDT_URL,
