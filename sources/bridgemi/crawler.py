#!/usr/bin/env python3
"""
Bridge Michigan - Talent & Education 爬虫
从 https://bridgemi.com/category/talent-education/ 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
BRIDGEMI_URL = "https://bridgemi.com/category/talent-education/"
RAW_DIR = "data/raw/bridgemi"
...
def categorize(title):
    ...
    categories = {
        'policy': ['secretary', 'education', 'tax credit', 'schools', 'whitmer'],
        'politics': ['political', 'governor', 'federal', 'state'],
        'education': ['school', 'scholarship', 'students', 'k-12'],
        'michigan': ['michigan', 'local']
    }
...
def crawl_bridgemi():
    """从 Bridge Michigan 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Bridge Michigan - Talent & Education...")
    print(f"📡 网址: {BRIDGEMI_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(BRIDGEMI_URL, headers=headers, timeout=20)
        ...
        for item in article_items:
            try:
                ...
                # 作者
                author_elem = item.find('a', class_='url fn n')
        ...
        output = {
            'source': 'Bridge Michigan - Talent & Education',
            'source_url': BRIDGEMI_URL,
