#!/usr/bin/env python3
"""
Education Minnesota 爬虫
从 https://educationminnesota.org/news/education-headlines/ 抓取最新新闻
只抓取最近4天内的文章
"""
import json
import requests
...
EDMN_URL = "https://educationminnesota.org/news/education-headlines/"
RAW_DIR = "data/raw/education_minnesota"
...
def categorize(title):
    ...
    categories = {
        ...
        'policy': ['policy', 'legislation', 'campaign'],
        'educators': ['educator', 'teacher', 'staff'],
        'union': ['union', 'association', 'member']
    }
...
def crawl_education_minnesota():
    """从 Education Minnesota 抓取最新新闻（最近4天）"""
    print("🚀 开始抓取 Education Minnesota...")
    print(f"📡 网址: {EDMN_URL}")
    print(f"📅 只抓取最近4天内的文章")
    ...
    try:
        ...
        response = requests.get(EDMN_URL, headers=headers, timeout=20)
        ...
        # 查找所有文章 - article.kt-blocks-post-grid-item
        article_items = soup.find_all('article', class_='kt-blocks-post-grid-item')
        ...
        for item in article_items:
            try:
                ...
                # 日期 - 从time标签获取
                date_str = ""
                time_elem = item.find('time', class_='kt-blocks-post-date')
                if time_elem:
                    # 尝试从datetime属性获取
